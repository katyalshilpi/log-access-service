...
public class SmokeIT {
    ...
    /**
     * Used to access endpoints during testing.
     * Adjusts URIs for host and port so that they can be specified by path.
     */
...
//        assertThat(response.getStatusCode().is4xxClientError()).isTrue();
    ...
    /**
     * Generate client HTTP requests converted for the appropriate host and port.
     * Host and port are defined by methods on the parent class.
     */
    @VisibleForTesting
    class AdjustedClientHttpRequestFactory extends SimpleClientHttpRequestFactory {
        ...
        URI adjustUri(URI uri) throws URISyntaxException {
            return new URI("http", uri.getUserInfo(),
                    host(), port(), uri.getPath(), uri.getQuery(), uri.getFragment());
        }
    }
}
